...
import type { LoopTurn } from "../fixtures";
...
// more:多余行数的提示文案(语言相关,从 i18n 传入)。
function clipLines(s: string, n: number, more: (k: number) => string): string {
  const lines = (s ?? "").split("\n");
  if (lines.length <= n) return s ?? "";
  return lines.slice(0, n).join("\n") + `\n${more(lines.length - n)}`;
}

type ParsedTool = { name: string; param: string };
type Node =
...
function Progression({ activeIter, loopCount }: { activeIter: number; loopCount: number }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 15, fontWeight: 700, color: "#94a3b8", letterSpacing: 0.5, marginBottom: 12 }}>{tr.loopProgress}</div>
...
const ACTOR = {
  user: { color: ACTOR_COLOR.user.main, label: "LLM" },
...
function Lane({ actor, children }: { actor: keyof typeof ACTOR; children: React.ReactNode }) {
  const tr = useT();
  const color = ACTOR[actor].color;
  // user 标签语言相关(用户/User);llm/agent 是中性词,保持原样。
  const label = actor === "user" ? tr.alActorUser : ACTOR[actor].label;
  return (
...
function NodeBox({ node, active, ctxStage, showTools, highlightTool, enter = 1, linkDraw = 1, linkHold = 0, typeT = 1 }: { node: Node; active: boolean; ctxStage: CtxStage; showTools: boolean; highlightTool: boolean; enter?: number; linkDraw?: number; linkHold?: number; typeT?: number }) {
  ...
  if (node.kind === "task") {
    ...
    return (
      <EventRow accent={ACTOR.user.color} label={tr.alTaskLabel} active={active} enter={enter}>
  ...
  if (node.kind === "response") {
    return (
      <EventRow accent={ACTOR.llm.color} label={tr.alLlmLabel} active={active} enter={enter}>
        {node.aiText
          ? <div style={{ fontSize: 21, color: "#374151", lineHeight: 1.5, marginBottom: showTools ? 12 : 0 }}>{node.aiText}</div>
          : (!showTools && <div style={{ fontSize: 20, color: "#94a3b8", fontStyle: "italic" }}>{tr.alDeciding}</div>)}
        {showTools && node.tools.map((t, i) => (
          <div key={i} style={{ border: `1px solid ${highlightTool ? "#6366f1" : "#e0e7ff"}`, borderRadius: 12, padding: "12px 14px", marginTop: i ? 8 : 0, background: "#f8f9ff", boxShadow: highlightTool ? "0 0 0 3px rgba(99,102,241,0.22)" : "none" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 18, color: "#6366f1" }}>{tr.toolVerb[t.name] ?? tr.toolVerbFallback}</span>
            ...
            {t.name === "Bash"
              ? <div style={{ background: "#0f172a", borderRadius: 10, padding: "12px 16px", fontFamily: "monospace", fontSize: 18, lineHeight: 1.65, color: "#e2e8f0", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                  <span style={{ color: "#5eead4" }}>$ </span>{clipLines(softWrapBash(t.param), 6, tr.moreLines)}
                </div>
              : <div style={{ fontFamily: "monospace", fontSize: 19, color: "#334155", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{clipLines(t.param, 6, tr.moreLines)}</div>}
          </div>
        ))}
      </EventRow>
    );
  ...
  return (
    <EventRow accent={ACTOR.agent.color} label={tr.alResultLabel} active={active} enter={enter}
      overlay={linkHold > 0.01 ? (
        // tool_use → tool_result 的执行联动(靛蓝):从上面的 tool_use 框甩进本结果框,末端水平箭头
        <FlowLink accent={ACTOR_COLOR.llm.main} hold={linkHold} label={tr.alFlowRun}
      ...
      {node.results.map((o, i) => (
        <div key={i} style={{ marginTop: i ? 12 : 0 }}>
          ...
          <div style={{ fontFamily: "monospace", fontSize: 19, color: "#334155", background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 12, padding: "12px 16px", whiteSpace: "pre-wrap", wordBreak: "break-word", lineHeight: 1.55 }}>
            {clipLines(o.output, 7, tr.moreLines) || tr.alEmpty}
          </div>
        </div>
      ))}
    </EventRow>
...
function FinalNode({ text, calls, active, enter = 1 }: { text: string; calls: number; active: boolean; enter?: number }) {
  ...
  return (
    <div style={{ borderRadius: 14, padding: "18px 22px", background: "#f0fdf4", border: "2px solid #16a34a", boxShadow: active ? "0 0 0 4px rgba(22,163,74,0.18)" : "0 0 0 3px rgba(22,163,74,0.1)" }}>
      <div style={{ fontSize: 18, fontWeight: 700, color: "#15803d", marginBottom: 10 }}>{tr.alFinalLabel(calls)}</div>
      <div style={{ fontSize: 21, color: "#14532d", lineHeight: 1.55, whiteSpace: "pre-wrap", wordBreak: "break-word", opacity: bodyOp, transform: `translateY(${bodyTy}px)` }}>{text || tr.alNoFinalText}</div>
    </div>
  );
}
...
function ContextBar({ iter, tokens, lastText, active, stage, inflow = 1, connHold = 0 }: { iter: number; tokens: number; lastText: string; active: boolean; stage: CtxStage; inflow?: number; connHold?: number }) {
  ...
  const linkLabel = assemble ? tr.flowAssemble : tr.flowStuffBack;
  ...
  return (
    <div style={{ position: "relative", border: `1px solid ${active ? "#6366f1" : "#eef2f6"}`, borderRadius: 12, background: "#fff", padding: "14px 18px", boxShadow: active ? "0 0 0 3px rgba(99,102,241,0.13)" : "none" }}>
      <div style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
        <span style={{ fontSize: 18, fontWeight: 700, color: "#334155" }}>{tr.ctxTitle}</span>
      ...
      <div style={{ fontSize: 15, color: "#9ca3af", marginTop: 8 }}>{tr.ctxPromptNote}</div>
      {full && iter > 0 && (
        <div style={{ marginTop: 8, background: "#f0fdfa", border: "1px solid #ccfbf1", borderRadius: 10, padding: "8px 14px" }}>
          <span style={{ fontSize: 16, fontWeight: 700, color: TEAL }}>{tr.ctxLatestResult}</span>{" "}
          <span style={{ fontSize: 18, color: "#374151", wordBreak: "break-word" }}>{lastText || tr.alEmpty}</span>
        </div>
      )}
    </div>
  );
