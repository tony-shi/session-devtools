        ...
        {stories.map((s, i) => (
          <div key={s.id} style={{ border: "1px solid #e2e8f0", borderRadius: 14, overflow: "hidden", background: "#fff" }}>
            ...
            >
              ...
              <span style={{ flex: 1 }}>
                <span style={{ display: "block", fontSize: 16, fontWeight: 700, color: "#0f172a" }}>{s.title}</span>
            ...
            <div style={{ padding: "10px 18px 14px" }}>
              {s.steps.map((st, j) => (
                <div key={j} style={{ display: "flex", gap: 12, padding: "7px 0", borderBottom: j < s.steps.length - 1 ? "1px dashed #eef2f6" : "none" }}>
                  ...
                  <span style={{ flex: 1, fontSize: 12, color: "#334155" }}>{st.lines[0]}<span style={{ color: "#94a3b8" }}> · {st.lines.length} 句</span></span>
                </div>
              ))}
            </div>
          </div>
