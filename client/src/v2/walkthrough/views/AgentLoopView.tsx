...
// Bash 长命令的可读换行:遇到 ;  &&  ||  | 时,把后面的标记推到下一行
// (用零宽前缀让 wordBreak 找到一个合法断点,而不是把单词截在中间)。
...
type ResultItem = { name: string; output: string; isError: boolean; truncated: boolean; fullSize: number };
type Node =
...
// outputPreview 已被 server 截断到 ~300 字符;若 outputSize 更大则视为截断,
// 显示时回退到最后一处空白/换行,避免把 URL 或单词切成两半。
function cleanTruncated(preview: string, fullSize: number): { display: string; truncated: boolean } {
  const raw = (preview ?? "").trim();
  if (fullSize <= raw.length) return { display: raw, truncated: false };
  const nl = raw.lastIndexOf("\n");
  const sp = raw.lastIndexOf(" ");
  const cut = Math.max(nl, sp);
  return { display: cut > 80 ? raw.slice(0, cut) : raw, truncated: true };
}

function buildNodes(turn: UserTurn): Node[] {
  const userInput = (turn.userInput ?? "").trim();

  // 不再裁剪迭代次数 —— 在 loop 一幕里把整个 Turn 完整展开,让"循环"真的循环起来。
  ...
  iters.forEach((c, i) => {
    ...
    nodes.push({
      kind: "result",
      iter: i,
      results: c.toolCalls.map((tc) => {
        const { display, truncated } = cleanTruncated(tc.outputPreview ?? "", tc.outputSize);
        return { name: tc.name, output: display, isError: tc.isError, truncated, fullSize: tc.outputSize };
      }),
    });
  });
          ...
          <div style={{ fontFamily: "monospace", fontSize: 13, color: "#334155", background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "8px 12px", whiteSpace: "pre-wrap", wordBreak: "break-word", lineHeight: 1.55 }}>
            ...
            {o.truncated && (
              <span style={{ color: "#94a3b8", fontStyle: "italic" }}>
                {"\n"}… 已截断 · 完整 {o.fullSize.toLocaleString()} 字符
              </span>
            )}
          </div>
        </div>
...
// 文案刻意强调"由 LLM 自行决策停止" —— 这是 Agent Loop 唯一的退出条件。
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 13, fontWeight: 700, color: "#15803d", marginBottom: 6 }}>
        ✅ LLM 自行决策停止 · 这一次不再 tool_use → 给出结论
      </div>
      <div style={{ fontSize: 14, color: "#14532d", lineHeight: 1.55, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{text || "(无最终文本)"}</div>
      <div style={{ fontSize: 12, color: "#16a34a", marginTop: 8, lineHeight: 1.55 }}>
        没有外部裁判,也没有固定轮数 —— 循环的退出完全由模型自己判断「信息已充分」。
        本轮共 {calls} 次 LLM 调用,Turn 到此终止。
      </div>
    </div>
  );
}
