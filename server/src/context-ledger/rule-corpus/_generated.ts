...
export const GENERATED_RULES: ReadonlyArray<Omit<Rule, "filePath">> = [
  ...
  },
  {
    ...
    "verifiedFor": "2.1.167",
    "appliesTo": {
      "minCcVersion": "2.1.167"
    },
    "sourceUnits": [],
    "description": "Task（Agent tool）general-purpose subagent 的 sys[2] 段（\"You are an agent for Claude Code…\"）。静态前缀 2205B prefix 锚定，终止于 env 壳 \"Working directory: \" 标签。仅覆盖 general-purpose——Explore（\"You are a file search specialist…\"）等其他 agent type 头部整段不同，待真实样本补 各自规则。billing header 带 cc_is_subagent=true（区分于 workflow agent）。",
    "stability": "static",
  ...
  },
  {
    ...
    "sourceUnits": [],
    "description": "Workflow subagent 的 sys[2] 段（\"You are a subagent spawned by a workflow orchestration script…\"）。静态前缀 1552B prefix 锚定，终止于 env 壳的 \"Working directory: \" 标签——其后是 session 动态值（cwd/git/model/gitStatus）。 跨项目、跨 2.1.167-2.1.168 逐字节验证（proxy req_011Cbqcmh6yceeiEY35fQCuD / req_011Cbqb4Bz1vJVw4fvzoXaqb）。注意 workflow agent 的 billing header 无 cc_is_subagent 标记（Task agent 有），无 H1 section 结构。",
    "stability": "static",
  ...
  },
  {
    ...
    "sourceUnits": [],
    "description": "Workflow subagent 专属工具：StructuredOutput（带 schema 的 agent() 强制其 调用一次返回结构化结果）。description 178B session-static、跨 agent/项目 逐字节一致，exact 锚定；input_schema 是 per-agent 动态的（编排脚本现场 定义的输出 shape），不入 pattern。",
    "stability": "static",
    ...
    "pattern": "Use this tool to return your final response in the requested structured format. You MUST call this tool exactly once at the end of your response to provide the structured output."
  },
  {
  ...
  {
    "ruleId": "claude-code.tool.Workflow.v2",
    "slotId": "tools.builtin",
    "verifiedFor": "2.1.170",
    "appliesTo": {
      "minCcVersion": "2.1.170"
    },
    "sourceUnits": [],
    "description": "Claude Code 工具：Workflow。2.1.170 模板，exact 全文锚定（static，跨同 session 相隔 2.5 天两条请求逐字节一致）。desc 18519 chars。相对 v1 （2.1.158）6 处语义变更：opt-in 触发词 workflow→ultracode、\"use a workflow\" 示例、agent() null 的 API 终态来源、parallel/pipeline 4096 items 上限等。 首变版本未核（2.1.159-2.1.169 无样本），minCcVersion 保守钉 2.1.170； v1 不加上界——其 verifiedFor 2.1.158 仍为真。",
    "stability": "static",
    "sourcemapRef": "proxy:req_011CbtoeoZ2ErDuDs31d8oys (2.1.170.005)；交叉验证 req_011Cbxx1AtArnpjyT3Apnedt",
    "materialization": "exact_text",
    "displayName": "Workflow",
    "summary": "工具定义：Workflow（2.1.170 版固定描述）",
    "priority": 10,
    "attribution": {
      "patternFromBody": true,
      "trailingNewlines": 0,
