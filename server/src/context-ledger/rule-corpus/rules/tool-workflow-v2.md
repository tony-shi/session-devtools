...
sourceUnits: []
description: >-
  Claude Code 工具：Workflow。2.1.170 模板，exact 全文锚定（static，跨同
  session 相隔 2.5 天两条请求逐字节一致）。desc 18519 chars。相对 v1
  （2.1.158）6 处语义变更：opt-in 触发词 workflow→ultracode、"use a workflow"
  示例、agent() null 的 API 终态来源、parallel/pipeline 4096 items 上限等。
  首变版本未核（2.1.159-2.1.169 无样本），minCcVersion 保守钉 2.1.170；
  v1 不加上界——其 verifiedFor 2.1.158 仍为真。
stability: static
...
与 tool-workflow-v1（2.1.158）并存：exact pattern 互斥，appliesTo 重叠区只会
命中文本一致的那条。
